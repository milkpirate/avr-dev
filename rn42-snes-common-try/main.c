...
#undef __AVR_ATtiny24__
#undef __AVR_ATtiny25__
...
#endif // __AVR_ATtiny24__
#ifdef __AVR_ATtiny25__
	// basic defines
	#define F_CPU		11520000UL
	#define BAUD		(F_CPU / 100)
	#define OSCCAL_VAL	(189)		// 11.52 MHz calibration
...
#define NAME_STR_GAMEPAD	"Gamepad-" DEV_ID "\r"
#define NAME_STR_KEYBOARD	"Keyboard-" DEV_ID "\r"
...
/*	needs to be after definition of F_CPU
	to work properly				*/
...
void init_rn42(uint8_t gamepad)
{
    ...
    delay_ms(RN42_CMD_DLY);
		...
		delay_ms(RN42_CMD_DLY);
	#endif // defined(__AVR_ATtiny24__) || defined(__AVR_ATtiny25__)

    uart_tx_str_P(PSTR("S=,5500\r"));	// remap disconnect key (0x00) to 'U' (0x55)
    delay_ms(RN42_CMD_DLY);

	uart_tx_str_P(PSTR("SN,SNES-BT-"));	// set device name
	if (gamepad != 0)	uart_tx_str_P(PSTR(NAME_STR_GAMEPAD));
	else				uart_tx_str_P(PSTR(NAME_STR_KEYBOARD));
    delay_ms(RN42_CMD_DLY);
...
void init_uart(void)
{
	...
	#if defined(__AVR_ATtiny13__) || defined(__AVR_ATtiny24__)
		TIMSK0 &= ~(1<<OCIE0A);		// turn compare match interrupt off
		TIFR0   =  (1<<OCF0A);		// set compare match flag (to be on the safe side)
	#endif // defined(__AVR_ATtiny13__) || defined(__AVR_ATtiny24__)
	#ifdef __AVR_ATtiny25__
