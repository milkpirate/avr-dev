...
#define SLAVE_ADDR_ATTINY	0b10100000	// 0x50 ~> addr = (0x50>>1) = 0x28
#define buffer_size			255			//in bytes (2..254), change ONLY here!!!!!

volatile uint8_t rxbuffer[4];	// Buffer to write data received from the master
volatile uint16_t buffer_adr;	// Virtual buffer address register

#if 	(buffer_size > 1023)
		#error Buffer to big! Maximal 1023 Bytes.
#elif 	(buffer_size < 2)
